    ...
    public class MyUserController : Controller
    {
        ...
        public ActionResult Login(UserLogin userLogin, string ReturnUrl = "")
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                string password = UltilPass.Hash(userLogin.Password);
                var isValiUser = Membership.ValidateUser(userLogin.Username, password);


                PhoneStoreEntities p = new PhoneStoreEntities();
        ...
        public ActionResult Register([Bind(Exclude = "activeFlag")] user user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                user.password = UltilPass.Hash(user.password);
                #endregion


                #region Save data database
                using (PhoneStoreEntities phone_Store = new PhoneStoreEntities())
                {
        ...
        {
            bool Status = false;
            using (PhoneStoreEntities phone = new PhoneStoreEntities())
            {
        ...
        public ActionResult ForgotPassWord(string email)
        {
            string message = "";
            bool status = false;
            using (PhoneStoreEntities p = new PhoneStoreEntities())
            {
        ...
        {
            PhoneStoreEntities phone_Store = new PhoneStoreEntities();
        ...
        public void SendVerificationSendLinkEmail(string email, int id)
        {
            ...
            var fromemail = new MailAddress("16130287@st.hcmuaf.edu.vn", "Anh nguyen");//từ email này
            var toEmail = new MailAddress(email);//gửi nội dung đến email này
            var fromEmailPass = "faner@170598@";
